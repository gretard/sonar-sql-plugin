...
public enum SnowflakeRules {
	...
	protected Rule getMaterializedView() {
		...
		rImpl.getViolatingRulesCodeExamples().getRuleCodeExample().add("CREATE VIEW V1 (C1) AS SELECT 1 as C1;");
	...
	public Rule getUnusedJoinRule() {
		...
		impl.getCompliantRulesCodeExamples().getRuleCodeExample().add("SET $size = 'XSMALL'; SELECT $size; ");
	...
	protected Rule getInsertRule() {
		...
		RuleImplementation child2 = new RuleImplementation();
		child2.getNames().getTextItem().add(Column_list_in_parenthesesContext.class.getSimpleName());
