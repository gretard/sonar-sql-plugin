...
public enum BaseRules {
    ...
    public Rule getSelectAllRule() {
        ...
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT t1.*, t2.* from dbo.test as t1 inner join dbo.test2 as t2 on"
                                + " t1.id=t2.id;");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT name, surname from dbo.test;");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT name, surname, 1 * 3 from dbo.test;");
    ...
    public Rule getInsertRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>INSERT statement does not have columns listed. Always use a"
                        + " column list in your INSERT statements.</p>");
        rule.setTag("design");
        rule.setSeverity("MINOR");
        rule.setRemediationFunction("LINEAR");
        rule.setDebtRemediationFunctionCoefficient("2min");
        ...
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add("INSERT INTO dbo.test VALUES (1,2);\r\nINSERT INTO dbo.test2 VALUES (1,2);");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("INSERT INTO dbo.test (a,b) VALUES (1,2);");
    ...
    public Rule getOrderByRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Do not use column numbers in the ORDER BY clause. Always"
                    + " use column names in an order by clause. Avoid positional references.</p>");
        rule.setTag("design");
        rule.setSeverity("MINOR");
        rule.setRemediationFunction("LINEAR");
        rule.setDebtRemediationFunctionCoefficient("2min");
        ...
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT * from dbo.test order by 1, 2;");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT * from dbo.test order by name;");
    ...
    public Rule getExecRule() {
        ...
        rule.setDescription(".");
        rule.setDescription(
                "<h2>Description</h2><p>EXECUTE/EXEC for dynamic query was used. It is better to"
                        + " use sp_executesql for dynamic queries.</p>");
        ...
        impl.setRuleViolationMessage(
                "EXECUTE/EXEC for dynamic query is used. It is better to use sp_executesql for"
                        + " dynamic queries.");

        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("EXECUTE sp_executesql N'select 1';");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("exec sys.sp_test  @test = 'Publisher';");
    ...
    public Rule getSchemaRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Always use schema-qualified object names to speed up"
                        + " resolution and improve query plan reuse.</p>");

        RuleImplementation impl = new RuleImplementation();
        impl.setRuleViolationMessage("Always use schema-qualified object names");
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT * from test order by 1;");
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT OrderID, OrderDate FROM Orders");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT * from dbo.test order by name;");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT * from main.dbo.test order by name;");
        rule.setRuleImplementation(impl);
        return rule;
    }
    ...
    public Rule getNoLockRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Use of NOLOCK might cause data inconsistency"
                        + " problems.</p>");
    ...
    public Rule getCursorRule() {
        ...
        rule.setDescription(
                "Cursor lifecycle is violated. Cursor either is not opened, deallocated or"
                        + " closed.");
        ...
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "DECLARE vend_cursor CURSOR      FOR SELECT * FROM Purchasing.Vendor; OPEN"
                                + " vend_cursor; FETCH NEXT FROM vend_cursor; SELECT * FROM"
                                + " Purchasing.Vendor; DEALLOCATE vendor_cursor; ");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "DECLARE vend_cursor CURSOR      FOR SELECT * FROM Purchasing.Vendor; OPEN"
                            + " vend_cursor; FETCH NEXT FROM vend_cursor; SELECT * FROM"
                            + " Purchasing.Vendor; CLOSE vend_cursor; DEALLOCATE vend_cursor; ");
    ...
    public Rule getMultipleDeclarations() {
        ...
        parent.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "DECLARE vend_cursor CURSOR      FOR SELECT * FROM"
                            + " Purchasing.Vendor;DECLARE vend_cursor CURSOR      FOR SELECT * FROM"
                            + " Purchasing.Vendor; OPEN vend_cursor; BEGIN FETCH NEXT FROM"
                            + " vend_cursor; END SELECT * FROM Purchasing.Vendor; BEGIN CLOSE"
                            + " vend_cursor; END DEALLOCATE vend_cursor; ");
        parent.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "DECLARE vend_cursor CURSOR      FOR SELECT * FROM Purchasing.Vendor; OPEN"
                            + " vend_cursor; FETCH NEXT FROM vend_cursor; SELECT * FROM"
    ...
    public Rule getSameFlow() {
        ...
        rule.setDescription(
                "Cursor was closed or deallocated in a different control statement than declared.");
        ...
        parent.getViolatingRulesCodeExamples()
                ...
                .add(
                        "DECLARE vend_cursor CURSOR      FOR SELECT * FROM Purchasing.Vendor; OPEN"
                            + " vend_cursor; BEGIN FETCH NEXT FROM vend_cursor; END SELECT * FROM"
                            + " Purchasing.Vendor; BEGIN CLOSE vend_cursor; END DEALLOCATE"
                            + " vend_cursor; ");
        parent.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
    ...
    public Rule getPKRule() {
        ...
        ruleImpl.setRuleViolationMessage(
                "Defined primary key is not using recommended naming convention");
        ruleImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "CREATE TABLE dbo.Orders\r\n"
                                ...
                                + ");  ");
        ruleImpl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "ALTER TABLE dbo.Orders ADD CONSTRAINT PK_OrderId PRIMARY KEY CLUSTERED"
                                + " (Id);");
        ruleImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Defined primary key is not using recommended naming"
                        + " convention to start with PK_.</p>");
        rule.setTag("naming");
        rule.setSeverity("MINOR");
        rule.setRemediationFunction("LINEAR");
    ...
    public Rule getFKRule() {
        ...
        ruleImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "ALTER TABLE dbo.Orders ADD CONSTRAINT ClientId FOREIGN KEY (ClientId)"
                                + " REFERENCES dbo.Clients(Id);  ");
        ruleImpl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "ALTER TABLE dbo.Orders ADD CONSTRAINT FK_ClientId FOREIGN KEY (ClientId)"
                                + " REFERENCES dbo.Clients(Id); ");
        ruleImpl.setRuleViolationMessage(
                "Defined foreign key is not using recommended naming convention");
        Rule rule = new Rule();
        rule.setKey("C011");
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Defined foreign key is not using recommended naming"
                        + " convention to start with FK_.</p>");
        rule.setTag("naming");
        rule.setSeverity("MINOR");
        rule.setRemediationFunction("LINEAR");
        rule.setDebtRemediationFunctionCoefficient("3min");
    ...
    public Rule getIndexNamingRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Defined index name is not using recommended naming"
                        + " convention to start with IX_.</p>");
        rule.setTag("naming");
        rule.setSeverity("MINOR");
        rule.setRemediationFunction("LINEAR");
        rule.setDebtRemediationFunctionCoefficient("3min");
    ...
    public Rule getNullComparisonRule() {
        ...
        r.setDescription(
                "<h2>Description</h2><p>It is not advisable to use comparison operator to check if"
                    + " value is null as comparison operators return UNKNOWN when either or both"
                    + " arguments are NULL. Please use IS NULL or IS NOT NULL instead.</p>");
        r.setSeverity("MAJOR");
        ...
        r.getRuleImplementation()
                ...
                .add("SELECT * from dbo.test where name <> null;");
        r.getRuleImplementation()
                .setRuleViolationMessage(
                        "Comparison operator (=, <>, !=) to check if value is null used");
    ...
    public Rule getWhereWithOrVsUnionRule() {
        ...
        r.setDescription(
                "<h2>Description</h2><p>It is  advisable to consider using UNION/UNION ALL operator"
                        + " instead of OR verb in the WHERE clause.</p>");
        r.setSeverity("MAJOR");
        ...
        rImpl.setRuleViolationMessage(
                "Consider using UNION/UNION ALL instead of OR in a WHERE clause.");
        r.setRuleImplementation(rImpl);
        rImpl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname, count from dbo.test where name = 'or' and surname ="
                                + " 'TestOR';");
        rImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname, count from dbo.test where name = 'Test' OR surname ="
                                + " 'Testor';");
        r.setSource(
                "https://dzone.com/articles/sql-handbook-and-best-practices-performance-tuning");
        return r;
    }
    ...
    public Rule getUnionVsUnionALLRule() {
        ...
        r.setDescription(
                "<h2>Description</h2><p>It is  advisable to consider using UNION ALL operator"
                        + " instead of UNION.</p>");
        r.setSeverity("MAJOR");
        r.setTag("performance");
        r.setRemediationFunction("LINEAR");
        ...
        rImpl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname, count from dbo.test union all SELECT name, surname,"
                                + " count from dbo.test2;");
        rImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname, count from dbo.test union SELECT name, surname,"
                                + " count from dbo.test2;");
        r.setSource(
                "https://dzone.com/articles/sql-handbook-and-best-practices-performance-tuning");
        r.setRuleImplementation(rImpl);
    ...
    public Rule getExistsVsInRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Consider using EXISTS/NOT EXISTS operator instead of IN for"
                        + " a subquery.</p>");
        ...
        ruleImplementation.setRuleViolationMessage(
        ...
        ruleImplementation
                ...
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname, count from dbo.test where exists (select 1 from"
                                + " dbo.locations where id = locationID);");
        ruleImplementation
                .getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname, count from dbo.test where locationID in (select id"
                                + " from dbo.locations);");
    ...
    public Rule getOrderByRuleWithoutAscDesc() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>It is advisable to specidy order how rows should be"
                        + " ordered.</p>");
    ...
    public Rule getSargRule() {
        ...
        rule.setDescription(
                "<h2>Description</h2><p>Use of non-sargeable arguments might cause performance"
                        + " problems.</p>");
        rule.setTag("performance");
        rule.setSeverity("MAJOR");
        rule.setRemediationFunction("LINEAR");
        ...
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT MAX(RateChangeDate)  FROM HumanResources.EmployeePayHistory WHERE"
                                + " BusinessEntityID = 1");
        impl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
        ...
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, surname from dbo.test where date between 2008-10-10 and"
                                + " 2010-10-10;");
        impl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
    ...
    protected Rule getCartesianJoinsRule() {
        ...
        rImpl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add("SELECT name, surname FROM customers c");

        rImpl.getCompliantRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, bank_name FROM customers c  inner join banks b on c.bank_id ="
                                + " b.id");
        rImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT name, order_id FROM customers, orders WHERE customers.id ="
                                + " orders.customer_id;");
    ...
    public Rule getColumnListRule() {
        ...
        r.setDescription("<h2>Description</h2><p>Avoid implicit column references.</p>");
        r.setSeverity("MAJOR");
        r.setTag("readability");
        r.setRemediationFunction("LINEAR");
        r.setDebtRemediationFunctionCoefficient("3min");
        ...
        rImpl.getViolatingRulesCodeExamples()
                .getRuleCodeExample()
                .add(
                        "SELECT n.name, surname, count from names n inner join surnames s on"
                                + " n.surname_id = s.id;");
        r.setSource(
                "https://dzone.com/articles/sql-handbook-and-best-practices-performance-tuning");
        r.setRuleImplementation(rImpl);
